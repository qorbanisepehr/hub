...
export const fieldSchemas = {
    ...
    job_priority_1: z.string().min(1, "اولویت شغلی الزامی است.").max(100, "حداکثر ۱۰۰ کاراکتر."),
    available_start_date: z.string().min(1, "تاریخ شروع به کار الزامی است.").max(255, "حداکثر ۲۵۵ کاراکتر."),
    expected_monthly_salary: z.number().min(0, "حقوق نمی‌تواند منفی باشد.").nullable().optional(),
    minimum_hours_per_month: z.number().min(0, "ساعات کاری نمی‌تواند منفی باشد.").nullable().optional(),
    expected_hourly_salary: z.number().min(0, "حقوق ساعتی نمی‌تواند منفی باشد.").nullable().optional(),
    preferred_workplace: z.array(z.enum(PREFERRED_WORKPLACE_VALUES)).optional(),
    job_priority_2: z.string().max(100, "حداکثر ۱۰۰ کاراکتر.").optional(),
    other_information: z.string().max(2000, "حداکثر ۲۰۰۰ کاراکتر.").optional(),
    submitted_resume_before: z.boolean().optional(),
    interviewed_before: z.boolean().optional(),
    currently_employed: z.boolean().optional(),
} as const;
